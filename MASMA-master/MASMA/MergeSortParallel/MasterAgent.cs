    ...
    class MasterAgent : Agent
    {
                    ...
                    case "done-phase1":
                        ...
                        if (doneSplit == Utils.validagentPool.Count)
                        {
                            ...
                            for (int j = 0; j < Utils.agentPool.Count; j++)
                            {
                                ...
                                {
                                    Send(Utils.agentPool[j], "terminate2");
                                }
                            }
                            Utils.agentPool = Utils.validagentPool;
                            for (int j = 0; j < Utils.validagentPool.Count; j++)
                            {
                                Send(Utils.validagentPool[j], "sort-asignData");
                            }
                        }

                        break;
                    case "done-sort-asignData":
                        doneAssignData++;
                        if (doneAssignData == Utils.agentPool.Count - removedWork)
                        {
        ...
        private void MergeAndCompare()
        {
            if (Utils.agentPool.Count > 1)
            {
                ...
                {
                    Send(Utils.agentPool[i++], "merge-and-compare");
                }
            }
        }
